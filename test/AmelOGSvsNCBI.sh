...
if [[ $1 == "memcheck" ]]; then
  memcheckcmd="valgrind --leak-check=full --show-reachable=yes --suppressions=data/misc/libpixman.supp --suppressions=data/misc/libpango.supp --error-exitcode=1 --gen-suppressions=all"
fi
...
find ${tempfile} -name "*.html" -exec sed -i 's/-nan/nan/g' {} \;
find ${tempfile}.orig -name "*.html" -exec sed -i 's/-nan/nan/g' {} \;
