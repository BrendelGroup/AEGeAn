...
static void parse_options(int argc, char **argv, GaevalOptions *options)
{
    ...
    if (numargs < 2)
    {
        print_usage(stderr);
        fprintf(stderr,
                "error: must provide at least 2 input files, %d provided\n",
                numargs);
        exit(1);
    }
...
int main(int argc, char **argv)
{
    ...
    GtNodeStream *stream =
        gt_gff3_in_stream_new_unsorted(1, &options.alignfile);
    ...
    GtNodeStream *align_stream = stream;

    stream =
        gt_gff3_in_stream_new_unsorted(options.numgenefiles, options.genefiles);
    gt_gff3_in_stream_check_id_attributes((GtGFF3InStream *)stream);
    gt_gff3_in_stream_enable_tidy_mode((GtGFF3InStream *)stream);
    gt_queue_add(streams, stream);
