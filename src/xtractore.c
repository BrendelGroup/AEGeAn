 ...
 * to that feature.
 */
static void xt_print_feature_sequence(GtGenomeNode *gn, const GtUchar *sequence,
                                      GtUword seqlength,
                                 ...
                                 XtractoreOptions *options, GtError *error)
{
    ...
    for (opt =
             getopt_long(argc, argv + 0, optstr, xtractore_options, &optindex);
         opt != -1; opt = getopt_long(argc, argv + 0, optstr, xtractore_options,
                                      &optindex))
    {
        if (opt == 'd')
        {
        ...
        else if (opt == 'w')
        {
            ...
            {
                gt_error_set(error,
                             "could not convert width '%s' to an integer",
                             optarg);
            }
        }
    }
                                    ...
                                    GtUword seqlength)
{
    ...
    GtArray *regions = xt_get_regions(gn);
    GtUword nregions = gt_array_size(regions);
    GtUword length = 0;
    for (GtUword i = 0; i < nregions; i++)
    {
        ...
        if (region->s != fstrand)
        {
            ...
            fprintf(stderr,
                    "[xtractore] error: feature at %s[%lu, %lu] belongs to "
                    "a multifeature with strand inconsistencies\n",
                    gt_str_get(seqid), region->r.start, region->r.end);
            exit(1);
        }
        if (region->r.end > seqlength)
        {
            GtStr *seqid = gt_genome_node_get_seqid(gn);
            fprintf(
                stderr, "[xtractore] error: feature at %s[%lu, %lu] exceeds "
                        "sequence length of %lu\n",
                gt_str_get(seqid), region->r.start, region->r.end, seqlength);
            exit(1);
        }
    }
    ...
    char *outseq = gt_malloc(sizeof(char) * (length + 1));
...
static GtArray *xt_get_regions(GtGenomeNode *gn)
{
    GtArray *regions = gt_array_new(sizeof(XtractRegion));
    ...
    if (gt_feature_node_is_pseudo(fn))
    {
        ...
        for (current = gt_feature_node_iterator_next(iter); current != NULL;
             current = gt_feature_node_iterator_next(iter))
        {
            XtractRegion region;
            GtRange r = gt_genome_node_get_range((GtGenomeNode *)current);
            GtStrand s = gt_feature_node_get_strand(current);
            region.r = r;
            region.s = s;
    ...
    GtRange r = gt_genome_node_get_range((GtGenomeNode *)fn);
                                      ...
                                      XtractoreOptions *options)
{
    ...
    if (strcmp(type, "CDS") == 0 && strncmp(feat_seq, "ATG", 3) != 0 &&
        options->verbose)
    {
...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr,
                "[xtractore] error: must provide a feature annotation file "
                "(GFF3 format) and a sequence file (Fasta format)\n");
        xt_print_usage(stderr);
        return 1;
    }
    const char *featfile = argv[optind + 0];
    const char *seqfile = argv[optind + 1];

    GtQueue *streams = gt_queue_new();

    GtNodeStream *current_stream = gt_gff3_in_stream_new_unsorted(1, &featfile);
    ...
    GtNodeStream *last_stream = current_stream;
    ...
    GtHashmap *seqs_observed =
        gt_hashmap_new(GT_HASH_STRING, gt_free_func, NULL);
    GtStrArray *seqfastas = gt_str_array_new();
    gt_str_array_add_cstr(seqfastas, seqfile);
    GtSeqIterator *seqiter =
        gt_seq_iterator_sequence_buffer_new(seqfastas, error);
    GtUword featcounter = 0;
    char *seqdesc;
    const GtUchar *sequence;
    GtUword seqlength;
    while ((result = gt_seq_iterator_next(seqiter, &sequence, &seqlength,
                                          &seqdesc, error)) > 0)
    {
    ...
    for (GtUword i = 0; i < gt_str_array_size(gff3seqids); i++)
    {
        const char *seqid = gt_str_array_get(gff3seqids, i);
        if (gt_hashmap_get(seqs_observed, seqid) == NULL)
        {
            fprintf(stderr,
                    "[AEGeAn::Xtractore] warning: sequence '%s' contains "
