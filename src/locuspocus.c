...
static void parse_options(int argc, char **argv, LocusPocusOptions *options,
                          GtError *error)
{
    int opt = 0;
    int optindex = 0;
    ...
    const struct option lp_options[] = {
    ...
    for (opt = getopt_long(argc, argv + 0, optstr, lp_options, &optindex);
         opt != -1;
         opt = getopt_long(argc, argv + 0, optstr, lp_options, &optindex))
    {
        if (opt == 'c')
        {
            options->by_cds = 1;
        ...
        else if (opt == 'e')
        {
            ...
            {
                gt_error_set(error,
                             "cannot set 'skipends' and 'endsonly' options"
        ...
        else if (opt == 'i')
        {
            ...
            if (options->ilenfile == NULL)
            {
                gt_error_set(error, "could not open ilenfile file '%s'",
                             optarg);
            }
        }
        else if (opt == 'l')
        {
            ...
            {
                gt_error_set(error,
                             "could not convert delta '%s' to an integer",
                             optarg);
            }
        }
        else if (opt == 'm')
        {
            ...
            {
                gt_error_set(error,
                             "could not convert overlap '%s' to an integer",
                             optarg);
            }
        }
        else if (opt == 'n')
        ...
        else if (opt == 't')
        {
            ...
            if (options->transstream == NULL)
            {
                gt_error_set(error, "could not open transmap file '%s'",
                             optarg);
            }
        }
        else if (opt == 'u')
...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr,
                "[LocusPocus] error: must provide at least one GFF3 file "
    ...
    GtNodeStream *current_stream =
        gt_gff3_in_stream_new_unsorted(numfiles, (const char **)argv + optind);
    ...
    current_stream =
        agn_infer_parent_stream_new(last_stream, options.type_parents);
    gt_queue_add(streams, current_stream);
    last_stream = current_stream;
    ...
    if (options.refine)
    {
        current_stream = agn_locus_refine_stream_new(
            last_stream, options.delta, options.minoverlap, options.by_cds);
    ...
    {
        current_stream = agn_locus_map_stream_new(
            last_stream, options.genestream, options.transstream);
        gt_queue_add(streams, current_stream);
        last_stream = current_stream;
    }
    ...
    if (options.retain)
    {
        gt_gff3_out_stream_retain_id_attributes(
            (GtGFF3OutStream *)current_stream);
    }
    gt_queue_add(streams, current_stream);
    last_stream = current_stream;
