...
class RefSeqDB(fidibus.genomedb.GenomeDB):
    ...
    def format_fasta(self, instream, outstream, logstream=sys.stderr):
        for defline, sequence in fidibus.fasta.parse(instream):
            ...
            fidibus.fasta.format(sequence, linewidth=80, outstream=outstream)
...
def test_genome_download():
    ...
    ador_db = fidibus.test_registry.genome('Ador')
    ...
    amel_db = fidibus.test_registry.genome('Amel')
    testurl = ('https://ftp.ncbi.nlm.nih.gov/genomes/refseq/invertebrate/'
...
def test_proteins_download():
    ...
    db = fidibus.test_registry.genome('Ador', workdir='/home/gandalf/HymHub')
    testurl = ('https://ftp.ncbi.nlm.nih.gov/genomes/refseq/invertebrate/'
               'Apis_dorsata/all_assembly_versions/'
               'GCF_000469605.1_Apis_dorsata_1.3/'
...
def test_gdna_format():
    ...
    db = fidibus.test_registry.genome('Hsal', workdir='testdata/demo-workdir')
    ...
    conf = fidibus.test_registry.genome('Tcas')
    db = fidibus.test_registry.genome('Tcas', workdir='testdata/demo-workdir')
    db.preprocess_gdna(logstream=None, verify=False)
    ...
    conf = fidibus.test_registry.genome('Mmus')
    db = fidibus.test_registry.genome('Mmus', workdir='testdata/demo-workdir')
    db.preprocess_gdna(logstream=None, verify=False)
...
def test_annot_format():
    ...
    db = fidibus.test_registry.genome('Aech', workdir='testdata/demo-workdir')
    ...
    db = fidibus.test_registry.genome('Pbar', workdir='testdata/demo-workdir')
    ...
    db = fidibus.test_registry.genome('Ador', workdir='testdata/demo-workdir')
...
def test_protids():
    ...
    db = fidibus.test_registry.genome('Xtro')
...
def test_cleanup():
    ...
    db = fidibus.test_registry.genome('Vcar', workdir='testdata/demo-workdir')
...
def test_genbank():
    """Genbank: smoke test"""
    db = fidibus.test_registry.genome('Znev')
